#!/bin/bash
#$ -N qsub_coassembly_7.24_E1.3
#$ -o /u/scratch/b/big2258/coldcreekanvio/coassembly_files/onesamp_7.24_1.3

#TODO: change output and error files

#$ -e /u/scratch/b/big2258/coldcreekanvio/coassembly_files/onesamp_7.24_1.3
#$ -cwd 
#$ -l h_data=6.25G
#$ -l h_rt=23:00:00
#$ -l h_vmem=25G
...
R1s=`ls 01_QC/sample_E1_3-QUALITY_PASSED_R1.fastq | python -c 'import sys; print(",".join([x.strip() for x in sys.stdin.readlines()]))'`
R2s=`ls 01_QC/sample_E1_3-QUALITY_PASSED_R2.fastq | python -c 'import sys; print(",".join([x.strip() for x in sys.stdin.readlines()]))'`
...
megahit -1 $R1s -2 $R2s --k-max $KMAX_SIZE --min-contig-len $MIN_CONTIG_SIZE -m 0.85 -o 02_ASSEMBLY/7.21_E1.3 -t $NUM_THREADS
