...
R1s=`ls 01_QC/sample_E1_*-QUALITY_PASSED_R1.fastq | python -c 'import sys; print(",".join([x.strip() for x in sys.stdin.readlines()]))'`
R2s=`ls 01_QC/sample_E1_*-QUALITY_PASSED_R2.fastq | python -c 'import sys; print(",".join([x.strip() for x in sys.stdin.readlines()]))'`
